...
int main(int argc, char *argv[ ]) {
    ...
    if (!l_out.is_open()) { // interactive version?
        std::cout << "QHsmTst example, built on " __DATE__ " at " __TIME__ "\n"
        ...
        std::cout << "QHsmTst, output saved to " << argv[1] << std::endl;
        if (l_out) {
            l_out << "QHsmTst example, QP " QP_VERSION_STR << std::endl;
        }
        else {
            std::cout << "QHsmTst example, QP " QP_VERSION_STR << std::endl;
        }
