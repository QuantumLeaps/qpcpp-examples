...
#endif
//$endskip${QP_VERSION} ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
//$define${App::the_calc} vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
...
Q_STATE_DEF(Calc, ready) {
    ...
    switch (e->sig) {
        ...
        //${SMs::Calc::SM::on::ready}
        case Q_EXIT_SIG: {
            BSP::message("ready-EXIT;");
            status_ = Q_RET_HANDLED;
            break;
        }
        //${SMs::Calc::SM::on::ready::initial}
        case Q_INIT_SIG: {
            BSP::message("ready-INIT;");
            status_ = tran(&begin);
            break;
        }
        ...
        case DIGIT_1_9_SIG: {
            BSP::clear();
            BSP::insert(Q_EVT_CAST(CalcEvt)->key_code);
            status_ = tran(&int1);
            break;
        }
...
Q_STATE_DEF(Calc, begin) {
    ...
    switch (e->sig) {
        //${SMs::Calc::SM::on::ready::begin}
        case Q_ENTRY_SIG: {
            BSP::message("begin-ENTRY;");
            status_ = Q_RET_HANDLED;
            break;
        }
        //${SMs::Calc::SM::on::ready::begin}
        case Q_EXIT_SIG: {
            BSP::message("begin-EXIT;");
            status_ = Q_RET_HANDLED;
            break;
        }
...
Q_STATE_DEF(Calc, result) {
    ...
    switch (e->sig) {
        //${SMs::Calc::SM::on::ready::result}
        case Q_ENTRY_SIG: {
            BSP::message("result-ENTRY;");
            status_ = Q_RET_HANDLED;
            break;
        }
        //${SMs::Calc::SM::on::ready::result}
        case Q_EXIT_SIG: {
            BSP::message("result-EXIT;");
            status_ = Q_RET_HANDLED;
            break;
        }
        default: {
...
Q_STATE_DEF(Calc, operand1) {
    ...
    switch (e->sig) {
        //${SMs::Calc::SM::on::operand1}
        case Q_ENTRY_SIG: {
            BSP::message("operand1-ENTRY;");
            status_ = Q_RET_HANDLED;
            break;
        }
        //${SMs::Calc::SM::on::operand1}
        case Q_EXIT_SIG: {
            BSP::message("operand1-EXIT;");
            status_ = Q_RET_HANDLED;
            break;
        }
...
Q_STATE_DEF(Calc, zero1) {
    ...
    switch (e->sig) {
        //${SMs::Calc::SM::on::operand1::zero1}
        case Q_ENTRY_SIG: {
            BSP::message("zero1-ENTRY;");
            status_ = Q_RET_HANDLED;
            break;
        }
        //${SMs::Calc::SM::on::operand1::zero1}
        case Q_EXIT_SIG: {
            BSP::message("zero1-EXIT;");
            status_ = Q_RET_HANDLED;
            break;
        }
...
Q_STATE_DEF(Calc, int1) {
    ...
    switch (e->sig) {
        //${SMs::Calc::SM::on::operand1::int1}
        case Q_ENTRY_SIG: {
            BSP::message("int1-ENTRY;");
            status_ = Q_RET_HANDLED;
            break;
        }
        //${SMs::Calc::SM::on::operand1::int1}
        case Q_EXIT_SIG: {
            BSP::message("int1-EXIT;");
            status_ = Q_RET_HANDLED;
            break;
        }
...
Q_STATE_DEF(Calc, frac1) {
    ...
    switch (e->sig) {
        //${SMs::Calc::SM::on::operand1::frac1}
        case Q_ENTRY_SIG: {
            BSP::message("frac1-ENTRY;");
            status_ = Q_RET_HANDLED;
            break;
        }
        //${SMs::Calc::SM::on::operand1::frac1}
        case Q_EXIT_SIG: {
            BSP::message("frac1-EXIT;");
            status_ = Q_RET_HANDLED;
            break;
        }
...
Q_STATE_DEF(Calc, opEntered) {
    ...
    switch (e->sig) {
        //${SMs::Calc::SM::on::opEntered}
        case Q_ENTRY_SIG: {
            BSP::message("opEntered-ENTRY;");
            status_ = Q_RET_HANDLED;
            break;
        }
        //${SMs::Calc::SM::on::opEntered}
        case Q_EXIT_SIG: {
            BSP::message("opEntered-EXIT;");
            status_ = Q_RET_HANDLED;
            break;
        }
...
Q_STATE_DEF(Calc, operand2) {
    ...
    switch (e->sig) {
        //${SMs::Calc::SM::on::operand2}
        case Q_ENTRY_SIG: {
            BSP::message("operand2-ENTRY;");
            status_ = Q_RET_HANDLED;
            break;
        }
        //${SMs::Calc::SM::on::operand2}
        case Q_EXIT_SIG: {
            BSP::message("operand2-EXIT;");
            status_ = Q_RET_HANDLED;
            break;
        }
        ...
        case EQUALS_SIG: {
            ...
            if (eval(BSP::get_value(), KEY_NULL)) {
                status_ = tran(&result);
            }
        ...
        case OPER_SIG: {
            ...
            if (eval(BSP::get_value(), Q_EVT_CAST(CalcEvt)->key_code)) {
                status_ = tran(&opEntered);
            }
...
Q_STATE_DEF(Calc, zero2) {
    ...
    switch (e->sig) {
        //${SMs::Calc::SM::on::operand2::zero2}
        case Q_ENTRY_SIG: {
            BSP::message("zero2-ENTRY;");
            status_ = Q_RET_HANDLED;
            break;
        }
        //${SMs::Calc::SM::on::operand2::zero2}
        case Q_EXIT_SIG: {
            BSP::message("zero2-EXIT;");
            status_ = Q_RET_HANDLED;
            break;
        }
...
Q_STATE_DEF(Calc, int2) {
    ...
    switch (e->sig) {
        //${SMs::Calc::SM::on::operand2::int2}
        case Q_ENTRY_SIG: {
            BSP::message("int2-ENTRY;");
            status_ = Q_RET_HANDLED;
            break;
        }
        //${SMs::Calc::SM::on::operand2::int2}
        case Q_EXIT_SIG: {
            BSP::message("int2-EXIT;");
            status_ = Q_RET_HANDLED;
            break;
        }
...
Q_STATE_DEF(Calc, frac2) {
    ...
    switch (e->sig) {
        //${SMs::Calc::SM::on::operand2::frac2}
        case Q_ENTRY_SIG: {
            BSP::message("frac2-ENTRY;");
            status_ = Q_RET_HANDLED;
            break;
        }
        //${SMs::Calc::SM::on::operand2::frac2}
        case Q_EXIT_SIG: {
            BSP::message("frac2-EXIT;");
            status_ = Q_RET_HANDLED;
            break;
        }
...
Q_STATE_DEF(Calc, error) {
    ...
    switch (e->sig) {
        //${SMs::Calc::SM::on::error}
        case Q_ENTRY_SIG: {
            BSP::message("error-ENTRY;");
            status_ = Q_RET_HANDLED;
            break;
        }
        //${SMs::Calc::SM::on::error}
        case Q_EXIT_SIG: {
            BSP::message("error-EXIT;");
            status_ = Q_RET_HANDLED;
            break;
        }
        default: {
