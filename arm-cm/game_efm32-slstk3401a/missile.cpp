...
#endif
//$endskip${QP_VERSION} ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
//$define${App::AO_Missile} vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
namespace APP {
    ...
    switch (e->sig) {
        ...
        case TIME_TICK_SIG: {
            ...
            if (m_x + GAME_MISSILE_SPEED_X < GAME_TUNNEL_WIDTH) {
                ...
                ObjectImageEvt *oie = QP::QF::q_new<ObjectImageEvt>(MISSILE_IMG_SIG);
                oie->x   = m_x;
                oie->y   = m_y;
                oie->bmp = MISSILE_BMP;
    ...
    switch (e->sig) {
        ...
        case TIME_TICK_SIG: {
            ...
            if ((m_x >= GAME_SPEED_X) && (m_exp_ctr < 15U)) {
                ...
                oie->y   = static_cast<std::int8_t>(m_y - 4U); // y-pos
