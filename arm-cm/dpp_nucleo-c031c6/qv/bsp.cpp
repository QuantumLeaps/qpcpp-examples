...
bool QS::onStartup(void const *arg) {
    ...
    SystemCoreClockUpdate();
    USART2->BRR  =
        UART_DIV_SAMPLING16(SystemCoreClock, 115200U, UART_PRESCALER_DIV1);
    USART2->CR3  = USART_CR3_OVRDIS; // Overrun Disable
    USART2->CR2  = (0x0U << USART_CR2_STOP_Pos); // 1 stop bit
    USART2->CR1  =
        USART_CR1_UE             |   // USART Enable
        USART_CR1_RE             |   // Receiver Enable
        USART_CR1_TE             |   // Transmitter Enable
        USART_CR1_RXNEIE_RXFNEIE |   // RXNE/RXFIFO not empty Interrupt Enable
        (0x00000UL << USART_CR1_M_Pos) | // Word length - 8 bits
        USART_CR1_FIFOEN_Msk;        // FIFO mode enable
