/*
 * UVISION generated file: DO NOT EDIT!
 * Generated by: uVision version 5.40.0.0
 *
 * Project: 'project' 
 * Target:  'spy' 
 */

#ifndef RTE_COMPONENTS_H
#define RTE_COMPONENTS_H


/*
 * Define the Device Header File: 
 */
#define CMSIS_device_header "stm32c0xx.h"

/* Keil::Device:Startup@1.0.0 */
#define RTE_DEVICE_STARTUP_STM32C0XX    /* Device Startup for STM32C0 */
/* QuantumLeaps::RTEF&qpcpp:Event Processor&qep@8.0.0 */
#define RTE_RTEF_QPCPP_QEP      /* QP/C++ QEP */
/* QuantumLeaps::RTEF&qpcpp:Framework&qf@8.0.0 */
#define RTE_RTEF_QPCPP_QF       /* QP/C++ QF */
/* QuantumLeaps::RTEF&qpcpp:Real-Time Kernel&qk@8.0.0 */
#define RTE_RTEF_QPCPP_QK       /* QP/C++ QK kernel */
/* QuantumLeaps::RTEF&qpcpp:Software tracing&qs@8.0.0 */
#define RTE_RTEF_QPCPP_QS       /* QP/C++ QS */


#endif /* RTE_COMPONENTS_H */
