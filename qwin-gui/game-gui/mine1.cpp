...
static inline std::uint8_t MINE_ID(Mine1 const * const me) {
    return static_cast<std::uint8_t>(me - &Mine1::inst[0]);
}
...
#endif
//$endskip${QP_VERSION} ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
//$define${App::Mine1_getInst} vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
namespace APP {
...
Q_STATE_DEF(Mine1, used) {
    ...
    switch (e->sig) {
        ...
        case Q_EXIT_SIG: {
            ...
            MineEvt *mev = QP::QF::q_new<MineEvt>(MINE_DISABLED_SIG);
            mev->id = MINE_ID(this);
    ...
    switch (e->sig) {
        ...
        case TIME_TICK_SIG: {
            ...
            if ((m_x >= GAME_SPEED_X) && (m_exp_ctr < 15)) {
                ...
                oie->y   = static_cast<std::int8_t>(m_y - 4 + 2U); // y of explosion
    ...
    switch (e->sig) {
        ...
        case TIME_TICK_SIG: {
            ...
            if (m_x >= GAME_SPEED_X) {
                ...
                ObjectImageEvt *oie = QP::QF::q_new<ObjectImageEvt>(MINE_IMG_SIG);
                oie->x   = m_x;
                oie->y   = m_y;
                oie->bmp = MINE1_BMP;
                AO_Tunnel->POST(oie, this);
