...
constexpr std::uint8_t SHIP_WIDTH  {5U};
constexpr std::uint8_t SHIP_HEIGHT {3U};
...
#endif
//$endskip${QP_VERSION} ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
//$define${App::AO_Ship} vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
namespace APP {
...
Q_STATE_DEF(Ship, flying) {
    ...
    switch (e->sig) {
        ...
        case Q_ENTRY_SIG: {
            ...
            ScoreEvt *sev = QP::QF::q_new<ScoreEvt>(SCORE_SIG);
            sev->score = m_score;
        ...
        case TIME_TICK_SIG: {
            ...
            ObjectImageEvt *oie = QP::QF::q_new<ObjectImageEvt>(SHIP_IMG_SIG);
            oie->x   = m_x;
            oie->y   = m_y >> 2;
            oie->bmp = SHIP_BMP;
        ...
        case PLAYER_TRIGGER_SIG: {
            ObjectPosEvt *ope = QP::QF::q_new<ObjectPosEvt>(MISSILE_FIRE_SIG);
            ope->x = m_x;
    ...
    switch (e->sig) {
        ...
        case TIME_TICK_SIG: {
            ...
            if (m_exp_ctr < 15U) {
                ...
                ObjectImageEvt *oie = QP::QF::q_new<ObjectImageEvt>(EXPLOSION_SIG);
                oie->bmp = EXPLOSION0_BMP + (m_exp_ctr >> 2U);
                oie->x   = m_x; // x of explosion
                oie->y   = static_cast<std::int8_t>((m_y >> 2U) - 4U + SHIP_HEIGHT);
                AO_Tunnel->POST(oie, this);
                status_ = Q_RET_HANDLED;
            }
            //${AOs::Ship::SM::active::exploding::TIME_TICK::[else]}
            else {
                ScoreEvt *gameOver =QP::QF::q_new<ScoreEvt>(GAME_OVER_SIG);
